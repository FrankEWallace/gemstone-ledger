...
import { ArrowLeft, ChevronRight, ChevronDown, Download, ChevronsUpDown } from "lucide-react";
...
function exportCSV(txs: Transaction[], customerName?: string) {
  const rows = [
  ...
  a.href = url;
  const safeName = (customerName ?? "customer").replace(/\s+/g, "-").toLowerCase();
  a.download = `expenses-${safeName}-${format(new Date(), "yyyy-MM-dd")}.csv`;
  a.click();
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {filteredTxs.length > 0 && (
          <Button
            ...
            size="sm"
            className="h-8 text-xs gap-1.5"
            onClick={() => exportCSV(filteredTxs, customer?.name)}
          >
