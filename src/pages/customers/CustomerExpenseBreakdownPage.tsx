            ...
            <table className="w-full text-xs min-w-[520px]">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="px-3 py-2.5 text-left font-semibold tracking-wider uppercase text-[10px] text-muted-foreground hidden md:table-cell">Qty × Unit</th>
...
export default function CustomerExpenseBreakdownPage() {
  ...
  const { data: txs = [], isLoading } = useQuery({
    queryKey: ["transactions", activeSiteId, "expense", id, dateFrom, dateTo],
    queryFn: () =>
    ...
    <div className="p-4 lg:p-6 space-y-5 max-w-[960px] mx-auto">
        ...
        <div className="space-y-1">
          ...
          {customer && (
            <p className="text-sm text-muted-foreground">
              All expenses for {customer.name}, by category
            </p>
          )}
        </div>
      ...
      <div className="grid grid-cols-3 rounded-xl border border-border bg-card divide-x divide-border">
        ...
        <div className="px-5 py-4">
          ...
          <p className="font-display text-2xl font-bold tabular-nums mt-1">{txs.length}</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-border bg-card p-10 text-center text-muted-foreground text-sm">
          No expense transactions found for this customer in this period.
        </div>
      ) : (
        <div className="space-y-3">
          {grouped.map((g, idx) => (
