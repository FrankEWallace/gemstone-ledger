      ...
      >
        ...
        <div className="flex-1 min-w-0 space-y-1.5">
          ...
          <p className="text-[10px] text-muted-foreground">
            {transactions.length} transaction{transactions.length !== 1 ? "s" : ""} · {pct}% of total income
          </p>
        </div>
...
export default function IncomeBreakdownPage() {
  ...
  const { data: txs = [], isLoading } = useQuery({
    queryKey: ["transactions", activeSiteId, "income", dateFrom, dateTo],
    queryFn: () =>
      getTransactions(activeSiteId!, { type: "income", dateFrom, dateTo }),
  ...
  if (!activeSiteId) {
    return (
      <div className="p-6 flex items-center justify-center h-64 text-muted-foreground text-sm">
        Select a site to view income breakdown.
      </div>
    );
  }
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5 max-w-[960px] mx-auto">
      ...
      <div className="space-y-1">
        ...
        <h1 className="font-display text-2xl font-bold tracking-tight">Income Breakdown</h1>
        <p className="text-sm text-muted-foreground">All income transactions grouped by category</p>
      </div>
      ...
      <div className="grid grid-cols-3 rounded-xl border border-border bg-card divide-x divide-border">
        <div className="px-5 py-4">
          <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold">Total Income</p>
