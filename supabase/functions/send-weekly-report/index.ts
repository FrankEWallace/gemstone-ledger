...
async function buildSiteReport(siteId: string, siteName: string, from: string, to: string): Promise<string> {
  ...
  const graded = (prodLogs ?? []).filter((l: any) => l.grade_g_t != null);
  const avgGrade  = graded.length > 0
    ? graded.reduce((s: number, l: any) => s + l.grade_g_t, 0) / graded.length
    : null;
...
serve(async (req) => {
  // Auth: accept either the Vault-backed cron secret (scheduled invocations) or
  // a valid user JWT (manual triggers from the System Settings UI). verify_jwt
  // is disabled so the non-JWT cron secret reaches the handler.
